...
@property (nonatomic, readonly) ICDrawingUndoStack *undoStack;
...
+ (struct CGSize { float x1; float x2; })fullSizeForOrientation:(int)arg1;
+ (struct CGAffineTransform { float x1; float x2; float x3; float x4; float x5; float x6; })fullSizeOrientationTransform:(int)arg1;
...
+ (struct CGSize { float x1; float x2; })platformFullPixelSize;
+ (struct CGSize { float x1; float x2; })platformFullSize;
...
- (struct CGSize { float x1; float x2; })unrotatedFullSize;
- (id)visibleCommands;
