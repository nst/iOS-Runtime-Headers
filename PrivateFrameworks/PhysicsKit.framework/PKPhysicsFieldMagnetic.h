/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/PhysicsKit.framework/PhysicsKit
 */

@interface PKPhysicsFieldMagnetic : PKPhysicsField {
}

@property BOOL isTangential;
@property float minRadius;
@property struct CGPoint { float x1; float x2; } offset;

+ (id)fieldWithMagnet:(struct CGPoint { float x1; float x2; })arg1 strength:(float)arg2 minRadius:(float)arg3;
+ (id)fieldWithMagnet:(struct CGPoint { float x1; float x2; })arg1 strength:(float)arg2;

- (id)initWithMagnet:(struct CGPoint { float x1; float x2; })arg1 strength:(float)arg2 minRadius:(float)arg3;
- (BOOL)isTangential;
- (float)minRadius;
- (struct CGPoint { float x1; float x2; })offset;
- (void)setIsTangential:(BOOL)arg1;
- (void)setMinRadius:(float)arg1;
- (void)setOffset:(struct CGPoint { float x1; float x2; })arg1;

@end
