/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/StoreKitUI.framework/StoreKitUI
 */

@interface SKUIDeferredActivityItemProvider : UIActivityItemProvider {
    SKUIClientContext * _clientContext;
    id  _itemProvider;
    SKUIProductPageItem * _productPageItem;
}

@property (retain) SKUIClientContext *clientContext;
@property (copy) id itemProvider;
@property (readonly, copy) SKUIProductPageItem *productPageItem;

+ (id)placeholderItem;

- (void).cxx_destruct;
- (id)clientContext;
- (id)initWithProductPageItem:(id)arg1 clientContext:(id)arg2;
- (id)initWithProductPageItemProvider:(id)arg1 clientContext:(id)arg2;
- (id)initWithProductPageItemProvider:(id)arg1 clientContext:(id)arg2 placeholderItem:(id)arg3;
- (id)itemProvider;
- (id)productPageItem;
- (void)setClientContext:(id)arg1;
- (void)setItemProvider:(id)arg1;

@end
